...
def consulta():
    ...
    if form.validate_on_submit():
        ...
        database = Processos.query.filter(
            coluna.contains(form.campo_busca.data)).all()
...
def importaMassivo(data_import: dict[str, str], form):
    ...
    data_import.update({
        "numproc": processo,
        "data_distribuicao": datetime.strptime(data_import["data_distribuicao"], "%d/%m/%Y"),
        "data_cadastro": form.data_cadastro.data
    })
    
    data_import.update({"parte_contraria": "Não Consta",
                        "adv_contrario": "Não Consta"})

    if data_import.get("assunto", None):
        
        chk_asst = Varas.query.filter(Assuntos.assunto == data_import.get("assunto", None)).first()
        if not chk_asst:
            assunto = Assuntos(
                assunto=data_import["assunto"]
            )
    ...
    if data_import.get("juiz", None):
        ...
        chk_jz = Varas.query.filter(Juizes.juiz == data_import.get("juiz", None)).first()
        if not chk_jz:
            juiz = Juizes(
                juiz=data_import["juiz"]
            )
    ...
    if data_import.get('classe', None):
        ...
        chk_clss = Classes.query.filter(Classes.classe == data_import.get("classe", None)).first()
        if not chk_clss:
            classe = Classes(
                classe=data_import.get('classe', None)
            )
    ...
    if data_import.get('foro', None):
        ...
        if not chk_fr:
            foro = Foros(
                foro=data_import.get('foro', None)
            )
            to_add.append(foro)
    ...
    if data_import.get("vara", None):
        ...
        if not chk_vr:
            vara = Varas(
                vara=data_import.get("vara", None)
            )
            to_add.append(vara)
    ...
    if data_import['polo_ativo'] == form.cliente.data:
        ...
        parte = Partes(
            nome=data_import['polo_passivo'],
            cpf_cnpj=data_import['cpf_polo_passivo']

        )
        to_add.append(parte)
        data_import.update({"cliente": data_import['polo_ativo'],
        ...
        parte = Partes(
            nome=data_import['polo_ativo'],
            cpf_cnpj=data_import['cpf_polo_ativo']
